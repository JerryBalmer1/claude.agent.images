...
# The core pin this image was built at, so it can be read back from inside the image and
# compared with the gitlink (tests/Image.Tests.ps1).
COPY config/vendor.json /opt/leash/config/vendor.json
